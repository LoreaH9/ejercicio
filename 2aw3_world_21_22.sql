...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 14-11-2021 a las 20:18:35
...
-- Procedimientos
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `spAllCities` ()  NO SQL
select * from city$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `spAllUsers` ()  NO SQL
select * from users$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `spFindContinentCountries` (IN `pContinent` VARCHAR(50))  NO SQL
BEGIN
SELECT * FROM country WHERE country.Continent=pContinent;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `spFindIdCity` (IN `pId` INT)  NO SQL
select *
from city 
where city.ID=pId$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `spFindIdCountry` (IN `pCode` VARCHAR(6))  NO SQL
select * from country
where country.Code=pCode$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `spFindLanguageCountries` (IN `pLanguage` VARCHAR(20))  NO SQL
select * from countrylanguage
where countrylanguage.Language= pLanguage
and countrylanguage.IsOfficial= true$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `spFindMostPopulationCity` (IN `pCountryCode` VARCHAR(10))  NO SQL
select *
from city as city1
where city1.CountryCode= pCountryCode 
and city1.Population= (select max(city2.Population)
                        from city as city2
                        where city1.countryCode=city2.countryCode
                        group by city2.countryCode)$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `spFindOfficialLanguage` (IN `pLanguage` VARCHAR(50))  NO SQL
select * from countrylanguage
where countrylanguage.Language=pLanguage
and countrylanguage.IsOfficial=true
and countrylanguage.Percentage >30$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `spFindUserById` (IN `pIdUser` INT)  NO SQL
select * from users
where users.idUser=pIdUser$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `spGetUserData` (IN `pUserName` VARCHAR(255))  NO SQL
